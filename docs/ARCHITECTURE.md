...
- Отправляет команды игрока в бэкенд.
...
3. Игрок отправляет команду движения.
4. Бэкенд валидирует команду и передает ее платформе или симуляции.
5. Платформа возвращает результат.
6. ИИ получает состояние и отправляет действие вмешательства.
7. Бэкенд проверяет действие ИИ, обновляет поле и публикует событие.
8. Mobile App показывает новое состояние: объект перемещен, путь заблокирован или команда стала недоступна.
