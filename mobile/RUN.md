...
> Наша заглушка просто проигрывает присланные команды (годится для проверки приложения на обоих актёрах). В корне репозитория есть более «умная» альтернатива — `simulation-emulator/tcp_emulator.py --agent-mode auto`: она сама выбирает ходы за `agent`. Обе слушают порт `5055` по одному протоколу — для приложения без разницы, какую запускать.
